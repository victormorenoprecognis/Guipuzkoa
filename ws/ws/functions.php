...
function getAllPharmacies($token)
{
	...
	$queryParams = [
		'$filter' => "_tgl_tipoentidadid_value eq dd880fce-8925-eb11-a813-000d3ab9b331"
	];
		...
		foreach ($valueArray as $item) {
			if ($item['tgl_latitud'] || $item['tgl_longitud']) {
				$pharmacyDataObject = [
					...
					'status' => getPharStatus($item['statuscode']),
					'fax' => $item['fax'],
					'opening' => null,
					'openingS' => null,
					'openingD' => null,
...
function getPharmaciesByTown($lang, $location, $neighborhood, $userLat, $userLng, $program, $token)
{
	...
	$baseURL = 'https://cofg.api.crm4.dynamics.com/api/data/v9.2/accounts';
	$queryParams = [
		'$filter' => "_tgl_tipoentidadid_value eq dd880fce-8925-eb11-a813-000d3ab9b331 and tgl_poblacion eq '$location'"
	];
...
{
	$filterStr = "_tgl_tipoentidadid_value eq dd880fce-8925-eb11-a813-000d3ab9b331 and _tgl_barrioid_value eq $neighborhood";
...
{
	$year = date('Y');
	$startTime = $year . "-" . normalizeMonths($month) . "-" . $day . "T09:00:00Z";
	$endTime = $year . "-" . normalizeMonths($month) . "-" . $day . "T22:00:00Z";

	if ($guardtime == 0) {
		$filterStr = 'tgl_horaapertura gt ' . $startTime . ' and tgl_horacierre lt ' . $endTime .
			' and _tgl_zonaguardia_value eq ' . $guardzone;
	} elseif ($guardtime == 1) {
		$filterStr = 'tgl_horaapertura gt ' . $startTime . ' and tgl_fecha eq ' . $endTime . ' and _tgl_zonaguardia_value eq ' . $guardzone;
	}
...
function getPharStatus($status)
{
	return ($status == "288710003") ? "O" : "C";
}
	...
	foreach ($arraylist as $pharId) {
		...
		if ($pharData && $pharTimetable) {
			$favPharData = [
				'id' => $pharData['id'],
				'name' => $pharData['name'],
				'address_num' => $pharData['tgl_direccion'],
				...
				'status' => getPharStatus($pharData['status']),
				'fax' => $pharData['fax'],
